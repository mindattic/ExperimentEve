// Collision is 2D in the XZ plane: bodies are circles, the world is AABBs and
// wall segments. Engine-agnostic on purpose — it only ever touches x/z, so it
// works against a Babylon Vector3 or any plain {x, z}.
...
/** Anything with mutable x/z — a Babylon Vector3 satisfies this. */
export interface XZ {
  x: number;
  z: number;
}
...
function pushOutAABB(pos: XZ, r: number, box: AABBCollider): boolean {
...
function pushOutSegment(pos: XZ, r: number, seg: SegmentCollider): boolean {
